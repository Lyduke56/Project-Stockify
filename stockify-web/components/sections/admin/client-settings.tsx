...
const inputStyle = "w-full bg-background border-[1.5px] border-primary rounded-2xl px-4 py-3 text-sm text-primary font-medium focus:outline-none hover:border-accent focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all placeholder:text-primary/30";
  ...
  return (
    <div className="w-full text-left">
      ...
      <div className="relative w-full">
        ...
        />
        {rightElement && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 text-primary/40 hover:text-primary transition-colors flex items-center justify-center">
            {rightElement}
          </div>
        )}
      </div>
...
export default function AdminSettingsModal({ isOpen, onClose, colors }: SettingsModalProps) {
        ...
        <>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 pointer-events-none">
            <motion.div
              ...
              className="w-full max-w-[920px] bg-background rounded-[32px] overflow-hidden border-[1.5px] border-accent/20 shadow-[0_32px_80px_rgba(58,97,49,0.2)] flex flex-col md:flex-row h-[650px] font-inter pointer-events-auto"
              style={{
            ...
            >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="flex items-center gap-4 transition-all duration-300 w-full text-left bg-transparent border-0 p-0 translate-x-2">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-accent text-primary shadow-lg shadow-accent/20">
              ...
              <div className="flex-1 flex flex-col relative bg-background backdrop-blur-sm">
                <button 
                  onClick={onClose} 
                  className="absolute top-6 right-6 w-10 h-10 rounded-full bg-background border border-primary flex items-center justify-center text-primary hover:bg-primary hover:text-background transition-all z-20 cursor-pointer"
                >
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    {activeTab === "security" && (
                      <motion.div key="security" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                        <div className="mb-8">
                          <span className="bg-accent/15 text-primary text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Admin Account</span>
                          <h3 className="text-2xl font-black text-primary mt-2 font-raleway italic">Security & Password</h3>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-5 max-w-md">
                          <InputField
                            ...
                            rightElement={
                              <button type="button" onClick={() => setShowCurrent(!showCurrent)} className="border-0 bg-transparent p-0 flex items-center cursor-pointer text-primary">
                          ...
                          <InputField
                            ...
                            rightElement={
                              <button type="button" onClick={() => setShowNew(!showNew)} className="border-0 bg-transparent p-0 flex items-center cursor-pointer text-primary">
                          ...
                          <InputField
                            ...
                            rightElement={
                              <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="border-0 bg-transparent p-0 flex items-center cursor-pointer text-primary">
                              ...
                              <div className="w-full h-1.5 bg-primary/10 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-300 ${
                                    ...
                                    : newPass.length < 10 ? "bg-accent w-1/2"
                                    : "bg-primary w-full"
                                  }`}
                                />
                              </div>
                          ...
                          <AnimatePresence>
                            ...
                            {passSaved && (
                              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-primary/10 border border-primary/20 rounded-2xl p-4 text-left flex items-center gap-2">
                                <Check size={16} className="text-primary" />
                                <p className="text-primary text-xs font-bold m-0">Password updated successfully.</p>
                              </motion.div>
                            )}
                          </AnimatePresence>
                        </div>
                ...
                <div className="px-8 py-5 border-t border-primary bg-background flex justify-between items-center z-20 shrink-0">
                  <button
                    onClick={onClose}
                    className="text-primary/50 text-sm font-bold hover:text-primary transition-colors cursor-pointer border-0 bg-transparent"
                  >
                    Cancel
                  </button>
                  ...
                  <button
                    ...
                    className="bg-primary text-background px-8 py-3 rounded-2xl text-sm font-bold flex items-center gap-2 hover:opacity-90 transition-opacity shadow-md disabled:opacity-60 disabled:cursor-not-allowed border-0 cursor-pointer"
                  >
