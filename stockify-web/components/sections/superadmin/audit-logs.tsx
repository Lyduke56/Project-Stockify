...
const SkeletonRow = ({ idx }: { idx: number }) => (
  <div className="w-full flex px-6 py-4 items-center gap-4 border-b border-[#385E31]/10" style={{ animationDelay: `${idx * 60}ms` }}>
      ...
      >
        ...
        <div className="w-full bg-[#FFFCEB] rounded-[10px] border border-[#385E31] flex flex-col overflow-visible shadow-sm">
          ...
          <div className="flex flex-col w-full py-1 min-h-[120px]">
            {/* Loading skeleton */}
            {loading && Array.from({ length: PAGE_SIZE }).map((_, i) => <SkeletonRow key={i} idx={i} />)}
            ...
            {!loading && error && (
              <div className="w-full flex flex-col items-center justify-center py-16 gap-3">
                ...
                <button onClick={() => fetchLogs(1, false)} className="text-[12px] font-bold text-[#385E31] underline underline-offset-2">
                  Retry
                </button>
              </div>
            )}
            ...
            {!loading && !error && logs.map((row, idx) => {
              ...
              return (
                ...
                >
                  <div className="w-[150px] shrink-0 text-[#3A6131] text-[12px] font-medium pt-0.5 leading-relaxed">{formatDate(row.created_at)}</div>
            ...
            {loadingMore && Array.from({ length: 5 }).map((_, i) => <SkeletonRow key={`more-${i}`} idx={i} />)}
          </div>
        </div>
        ...
        <div className="w-full flex justify-between items-center mt-5">
          <p className="text-[#385E31]/40 text-[12px] font-medium">
            {!loading && !error && logs.length > 0 ? `Showing ${logs.length} of ${total.toLocaleString()}` : ""}
          </p>
