...
const DEFAULT_SIZES: SizeInput[] = [
  { label: "Small",  price: "", is_default: false },
  { label: "Medium", price: "", is_default: true  },
  { label: "Large",  price: "", is_default: false },
];
...
const STEPS = [
  ...
  { id: 4, label: "Sizes & Variants",     icon: Ruler   },
];
...
export default function ProductModal({ mode, tenantId, productId, initial, onSave, onClose }: ProductModalProps) {
  ...
  const [recipe, setRecipe] = useState<{ item_id: string; item_type: "fnb" | "nfb"; amount: string; unit: string }[]>(
    initial?.recipe?.map((r: any) => ({ item_id: r.item_id, item_type: r.item_type, amount: String(r.amount), unit: r.unit })) ?? []
  );

  const [sizes, setSizes] = useState<SizeInput[]>(
    initial?.sizes?.length
      ? initial.sizes.map((s: any) => ({ label: s.label, price: String(s.price), is_default: s.is_default }))
      : DEFAULT_SIZES
  );
  ...
  const addRecipeRow = () => setRecipe((p) => [...p, { item_id: "", item_type: "fnb", amount: "", unit: "" }]);
  ...
  const addSize = () => setSizes((p) => [...p, { label: "", price: "", is_default: false }]);
  const updateSize = (idx: number, key: keyof SizeInput, val: string | boolean) => setSizes((p) => p.map((s, i) => i === idx ? { ...s, [key]: val } : s));
  const setDefaultSize = (idx: number) => setSizes((p) => p.map((s, i) => ({ ...s, is_default: i === idx })));
  const removeSize = (idx: number) => setSizes((p) => p.filter((_, i) => i !== idx));

  const handleSave = async () => {
    if (!form.name.trim() || !form.sku.trim()) { setError("Product name and SKU are required."); return; }
    const validRecipe: RecipeInput[] = recipe.filter((r) => r.item_id && Number(r.amount) > 0).map((r) => ({ item_id: r.item_id, item_type: r.item_type, amount: Number(r.amount), unit: r.unit }));
    const validSizes: SizeInput[] = sizes.filter((s) => s.label.trim());
    try {
      setSaving(true); setError(null);
      await onSave({ category_id: form.category_id || null, name: form.name.trim(), sku: form.sku.trim().toUpperCase(), description: form.description.trim() || null, image_url: imagePreview && !imageFile ? imagePreview : null, unit_cost: Number(form.unit_cost) || 0, price: Number(form.price) || 0, max_yield: Number(form.max_yield) || 0, visible: form.visible }, validRecipe, validSizes, imageFile);
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  <div className="flex justify-between items-start mb-2">
                    <div><span className="bg-[#F7B71D]/15 text-[#385E31] text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Step 02</span><h3 className="text-2xl font-black text-[#3A6131] mt-2 font-raleway italic">Ingredients Setup</h3></div>
                  ...
                  <p className="text-[11px] text-[#3A6131]/50 leading-relaxed pr-16">Map each ingredient and its quantity per serving.</p>
                  {loadingIngs ? <div className="flex items-center justify-center py-12 text-[#3A6131]/40 gap-2"><Loader2 size={20} className="animate-spin" /> Loading…</div> : (
                    <div className="space-y-3 mt-2">
                      {recipe.length === 0 ? (
                        <div className="py-12 border-2 border-dashed border-[#3A6131]/10 rounded-[24px] flex flex-col items-center justify-center text-[#3A6131]/30"><Coffee size={40} strokeWidth={1} className="mb-2" /><p className="text-sm font-medium">No ingredients mapped yet</p></div>
                      ) : recipe.map((row, idx) => (
                        <div key={idx} className="flex gap-3 items-center bg-white rounded-2xl p-3 border border-[#3A6131]/5 shadow-sm">
                          ...
                          <span className="text-[12px] font-black text-[#F7B71D] w-8 uppercase text-center">{row.unit || "—"}</span>
              ...
              {step === 3 && (
                <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  <div className="mb-6"><span className="bg-[#F7B71D]/15 text-[#385E31] text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Step 03</span><h3 className="text-2xl font-black text-[#3A6131] mt-2 font-raleway italic">Pricing & Metrics</h3><p className="text-[11px] text-[#3A6131]/50 mt-1">Base price — each size can override this in Step 4.</p></div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-[#3A6131]/5 p-6 rounded-[24px] border border-[#3A6131]/10"><label className={labelStyle}>Unit Cost (Internal)</label><div className="flex items-center text-2xl font-black text-[#3A6131]"><span className="mr-2 opacity-30">₱</span><input type="number" className="bg-transparent border-none p-0 focus:ring-0 w-full font-black" value={form.unit_cost} onChange={(e) => set("unit_cost", e.target.value)} placeholder="0.00" /></div></div>
                    <div className="bg-[#F7B71D] p-6 rounded-[24px] shadow-lg shadow-[#F7B71D]/20"><label className={`${labelStyle} text-[#385E31]/60`}>Base Selling Price</label><div className="flex items-center text-2xl font-black text-[#385E31]"><span className="mr-2 opacity-50">₱</span><input type="number" className="bg-transparent border-none p-0 focus:ring-0 w-full font-black placeholder:text-[#385E31]/30" value={form.price} onChange={(e) => set("price", e.target.value)} placeholder="0.00" /></div></div>
                    <div className="col-span-2"><label className={labelStyle}>Max Yield (Servings)</label><input type="number" className={inputStyle} value={form.max_yield} onChange={(e) => set("max_yield", e.target.value)} placeholder="e.g. 40" min="0" /></div>
                  </div>
              ...
              {step === 4 && (
                <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  <div className="flex justify-between items-start mb-2">
                    <div><span className="bg-[#F7B71D]/15 text-[#385E31] text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Step 04</span><h3 className="text-2xl font-black text-[#3A6131] mt-2 font-raleway italic">Sizes & Variants</h3><p className="text-[11px] text-[#3A6131]/50 mt-1 leading-relaxed pr-16">S/M/L are pre-filled. Add custom sizes, set price per size, mark one as default. Empty price uses the base price from Step 3.</p></div>
                  ...
                  <div className="flex gap-3 px-1 mt-6">
                    ...
                    <span className={`${labelStyle} w-28 mb-0 text-center`}>Price (₱)</span>
                    <span className={`${labelStyle} w-20 mb-0 text-center`}>Default</span>
                    <div className="w-9" />
                  </div>
                  <div className="space-y-2">
                    {sizes.length === 0 ? (
                      <div className="py-10 border-2 border-dashed border-[#3A6131]/10 rounded-[20px] flex flex-col items-center justify-center text-[#3A6131]/30"><Ruler size={36} strokeWidth={1} className="mb-2" /><p className="text-sm font-medium">No sizes — product uses base price</p></div>
                    ) : sizes.map((s, idx) => (
                      <div key={idx} className="flex gap-3 items-center bg-white rounded-2xl px-4 py-3 border border-[#3A6131]/5 shadow-sm">
                        <input className="flex-1 bg-transparent border-b border-[#3A6131]/15 text-[13px] font-bold text-[#3A6131] focus:outline-none focus:border-[#F7B71D] py-1" placeholder="e.g. Solo, Sharing…" value={s.label} onChange={(e) => updateSize(idx, "label", e.target.value)} />
                        <div className="w-28 flex items-center bg-[#3A6131]/5 rounded-xl px-3 py-1.5"><span className="text-[#3A6131]/30 text-sm font-black mr-1">₱</span><input type="number" className="w-full bg-transparent border-none p-0 focus:ring-0 text-[13px] font-black text-[#3A6131]" placeholder="—" value={s.price} onChange={(e) => updateSize(idx, "price", e.target.value)} /></div>
                        <div className="w-20 flex justify-center"><button onClick={() => setDefaultSize(idx)} className={`w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center ${s.is_default ? "border-[#3A6131] bg-[#3A6131]" : "border-[#3A6131]/30"}`}>{s.is_default && <div className="w-2 h-2 rounded-full bg-white" />}</button></div>
