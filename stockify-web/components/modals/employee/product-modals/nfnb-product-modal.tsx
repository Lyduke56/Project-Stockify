...
import {
  X, Info, ChevronRight,
  Trash2, Plus, Loader2, PackageCheck, Layers,
} from "lucide-react";
...
const EMPTY_OPTION  = (): VariantOptionInput => ({ label: "", price: "", stock: "", sku_suffix: "" });
...
export default function NfbProductModal({ mode, tenantId, initial, onSave, onClose }: NfbProductModalProps) {
  ...
  const [variants, setVariants] = useState<VariantTypeInput[]>(
  ...
  const updateVariantOption  = (ti: number, oi: number, key: keyof VariantOptionInput, val: string) =>
    setVariants((p) =>
      p.map((vt, i) =>
        ...
        <div className="w-full md:w-[300px] bg-[#3A6131] p-10 flex flex-col relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[#FFFCEB]/60 text-xs font-medium leading-relaxed mb-10">
              Set up your physical product and define optional variants.
            </p>
              ...
              {[1, 2].map((i) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all duration-500 ${
                    step === i ? "w-8 bg-[#F7B71D]" : "w-2 bg-white/20"
                  }`}
                />
              ))}
            </div>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-[11px] text-[#3A6131]/50 mt-1 leading-relaxed pr-8">
                        Add variant types like Size or Color. Each option has its own price and stock.
                      </p>
                    </div>
                    <button
                  ...
                  {variants.length === 0 ? (
                    <div className="py-12 border-2 border-dashed border-[#3A6131]/10 rounded-[24px] flex flex-col items-center justify-center text-[#3A6131]/30 mt-4">
                      ...
                      <p className="text-sm font-medium">No variants — product has single price &amp; stock</p>
                      ...
                      {variants.map((vt, ti) => (
                        <div key={ti} className="bg-white rounded-2xl border border-[#3A6131]/10 overflow-hidden shadow-sm">
                          ...
                          <div className="flex gap-2 px-4 pt-2 pb-1">
                            ...
                            <span className="w-24 text-[10px] font-black uppercase tracking-wider text-[#3A6131]/40 text-center">Price (₱)</span>
                            <span className="w-20 text-[10px] font-black uppercase tracking-wider text-[#3A6131]/40 text-center">Stock</span>
                            <span className="w-24 text-[10px] font-black uppercase tracking-wider text-[#3A6131]/40 text-center">SKU Suffix</span>
                            <div className="w-8" />
                          </div>
                          ...
                          <div className="space-y-1 px-4 pb-3">
                            {vt.options.map((opt, oi) => (
                              <div key={oi} className="flex gap-2 items-center">
                                ...
                                <input
                                  type="number"
                                  className="w-24 bg-[#FFFCEB]/60 border border-[#3A6131]/10 rounded-xl px-3 py-2 text-[12px] font-black text-[#3A6131] text-center focus:outline-none focus:border-[#F7B71D]"
                                  placeholder="0.00"
                                  value={opt.price}
                                ...
                                />
                                <input
                                  className="w-24 bg-[#FFFCEB]/60 border border-[#3A6131]/10 rounded-xl px-3 py-2 text-[12px] font-bold text-[#3A6131] text-center focus:outline-none focus:border-[#F7B71D]"
                                  placeholder="-SM"
                                  value={opt.sku_suffix}
                                  onChange={(e) => updateVariantOption(ti, oi, "sku_suffix", e.target.value)}
                                />
                                <button
