...
export default function TransactionDetailModal({ orderId, tenantId, onClose, colors }: TransactionDetailModalProps) {
  ...
  const cardStyle = "bg-white border-[1.5px] border-primary/10 rounded-2xl px-5 py-4 flex items-center gap-4 shadow-sm";
  ...
  return createPortal(
    <div style={modalStyles} className="fixed inset-0 z-[999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <motion.div 
      ...
      >
        ...
        <div className="flex-1 flex flex-col relative bg-background/50 backdrop-blur-sm overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-10">
                {loading ? (
                    <div className="h-full flex flex-col items-center justify-center gap-3 text-primary/40">
                        ...
                        {step === 1 && (
                            ...
                            >
                                <div className="mb-8">
                                    ...
                                    <h3 className="text-2xl font-black text-primary mt-2 font-raleway italic">Summary Details</h3>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-primary/5 p-6 rounded-[24px] border border-primary/10 flex flex-col justify-center">
                                        ...
                                        <p className="text-primary font-black text-lg truncate">{order.customer_name}</p>
                                    </div>
                                    <div className="bg-accent p-6 rounded-[24px] shadow-lg shadow-accent/20 flex flex-col justify-center items-end text-right">
                                        <label className={`${labelStyle} text-primary/60 !mb-1`}>Total Paid</label>
                                ...
                                <div className="pt-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <button 
                                            ...
                                            className={`flex items-center justify-between p-4 rounded-2xl border-[1.5px] transition-all ${order.proof_of_payment_url ? 'bg-background border-primary/15 hover:border-accent group shadow-sm' : 'bg-primary/5 border-transparent opacity-50 cursor-not-allowed'}`}
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[13px] font-bold text-primary">Payment</span>
                                            </div>
                                            {order.proof_of_payment_url && <Eye size={16} className="text-primary/30 group-hover:text-accent transition-colors" />}
                                        </button>

                                        <button 
                                            onClick={() => handleViewProof("PROOFS")}
                                            disabled={!order.delivery_proof_url}
                                            className={`flex items-center justify-between p-4 rounded-2xl border-[1.5px] transition-all ${order.delivery_proof_url ? 'bg-background border-primary/15 hover:border-accent group shadow-sm' : 'bg-primary/5 border-transparent opacity-50 cursor-not-allowed'}`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center text-purple-500"><Truck size={18} /></div>
                                                <span className="text-[13px] font-bold text-primary">Delivery</span>
                                            </div>
                                            {order.delivery_proof_url && <Eye size={16} className="text-primary/30 group-hover:text-accent transition-colors" />}
                                        </button>
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {step === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                <div className="mb-8">
                                    ...
                                    <h3 className="text-2xl font-black text-primary mt-2 font-raleway italic">Order Breakdown</h3>
                                </div>

                                <div className="bg-white rounded-2xl border border-primary/10 shadow-sm overflow-hidden">
                                    <div className="bg-primary/5 px-5 py-3 border-b border-primary/10">
                                        <h3 className="text-xs font-black text-primary uppercase tracking-wider">Purchased Items</h3>
                                    </div>
                                    <div className="max-h-[300px] overflow-y-auto">
                                        <table className="w-full text-sm">
                                            <thead className="bg-white text-primary/50 text-[10px] uppercase tracking-wider">
                                                <tr>
                                                ...
                                                {items.map((item, idx) => (
                                                    <tr key={idx} className="hover:bg-primary/5 transition-colors">
                                                        <td className="px-5 py-4 font-bold text-primary">
                                                            {item.item_name}
                                                            <span className="block text-[10px] text-primary/50 mt-0.5">₱{item.unit_price.toFixed(2)} each</span>
                                                        </td>
