      ...
      >
        <div className="bg-white rounded-[28px] w-full max-w-[500px] shadow-2xl pointer-events-auto overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {loading ? (
              <div className="h-40 flex flex-center flex-col items-center justify-center gap-3">
                ...
                <p className="text-[12px] text-primary/60 font-bold">Fetching Record...</p>
              </div>
            ) : order && (
              <>
                {/* Summary Cards */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                    ...
                    <p className="text-[14px] text-primary font-black">{order.customer_name}</p>
                  </div>
                  <div className="bg-accent/10 rounded-2xl p-4 border border-accent/20 text-right">
                    <p className="text-[10px] text-[#8a6700] font-bold uppercase mb-1">Total Paid</p>
                    <p className="text-[18px] text-primary font-black">₱{order.total_amount.toFixed(2)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl border transition-all ${order.proof_of_payment_url ? 'bg-white border-gray-200 hover:border-primary group' : 'bg-gray-50 border-gray-100 opacity-50 cursor-not-allowed'}`}
                    >
                      <div className="flex items-center gap-3">
                      ...
                      {order.proof_of_payment_url && <Eye size={14} className="text-gray-300 group-hover:text-primary" />}
                    </button>

                    <button 
                      onClick={() => handleViewProof("PROOFS")}
                      disabled={!order.delivery_proof_url}
                      className={`flex items-center justify-between p-3 rounded-xl border transition-all ${order.delivery_proof_url ? 'bg-white border-gray-200 hover:border-primary group' : 'bg-gray-50 border-gray-100 opacity-50 cursor-not-allowed'}`}
                    >
                      <div className="flex items-center gap-3">
                      ...
                      {order.delivery_proof_url && <Eye size={14} className="text-gray-300 group-hover:text-primary" />}
                    </button>
                  </div>
                </div>
                  ...
                  <div className="bg-gray-50 rounded-2xl overflow-hidden border border-gray-100">
                    {items.map((item, idx) => (
                      <div key={`${item.item_id}-${idx}`} className={`p-4 flex items-center justify-between ${idx !== items.length - 1 ? 'border-b border-gray-100' : ''}`}>
                        <div>
                          <p className="text-[13px] font-bold text-primary">{item.item_name}</p>
                          <p className="text-[10px] text-gray-400 font-medium">Qty: {item.quantity} × ₱{item.unit_price.toFixed(2)}</p>
                        </div>
                        <p className="text-[13px] font-black text-primary">₱{(item.quantity * item.unit_price).toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="p-6 bg-gray-50/80 border-t border-gray-100">
             <button onClick={onClose} className="w-full py-3 bg-primary text-white font-black rounded-xl hover:bg-secondary transition-colors shadow-lg shadow-primary/20">Close Receipt</button>
          </div>
        </div>
      </motion.div>
    </AnimatePresence>
  ...
  return (
    <div className="w-full flex flex-col font-['Inter'] px-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Transactions", value: transactions.length.toString(), color: "text-primary" },
          { label: "Total Revenue", value: `₱${totalRevenue.toLocaleString("en-PH", { minimumFractionDigits: 2 })}`, color: "text-accent" },
          { label: "Avg. Order Value", value: transactions.length ? `₱${(totalRevenue / transactions.length).toFixed(2)}` : "—", color: "text-purple-600" },
        ].map((card) => (
          <div key={card.label} className="bg-white rounded-2xl border border-primary/15 px-5 py-4 shadow-sm">
            <p className="text-[11px] font-black uppercase tracking-wider text-primary/40 mb-1">{card.label}</p>
      ...
      <div className="w-full flex justify-between items-center mb-4 gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setSortAsc(!sortAsc)}
            className="flex items-center gap-2 border border-primary text-primary px-4 py-2.5 rounded-full text-[13px] font-bold hover:bg-primary/5 transition-colors">
      ...
      <div className="w-full bg-background rounded-[10px] border border-primary flex flex-col overflow-hidden shadow-sm">
        <div className="w-full grid grid-cols-8 bg-primary px-4 py-3 rounded-t-[8px]">
          {COLUMNS.map((col) => (
            ...
            {filtered.map((txn, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`w-full grid grid-cols-8 px-4 py-[14px] items-center hover:bg-primary/3 transition-colors ${!isLast ? "border-b border-primary/10" : ""}`}
                >
                  <div className="text-center text-primary text-[11px] font-black font-mono">
                    {txn.transaction_id.slice(0, 8).toUpperCase()}
                  </div>
                  <div className="text-center text-primary/60 text-[11px] font-black font-mono">
                    {txn.order_id.slice(0, 8).toUpperCase()}
                  </div>
                  <div className="text-center text-primary/70 text-[12px] font-medium">{formatDate(txn.completed_at)}</div>
                  <div className="text-center text-primary text-[12px] font-bold truncate px-1">{txn.customer_name}</div>
                  <div className="text-center">
                    <span className="text-[11px] font-bold bg-primary/8 text-primary px-2.5 py-1 rounded-full">
                  ...
                  <div className="text-center text-accent text-[13px] font-black">
                    ₱{txn.total_amount.toFixed(2)}
                  </div>
                  <div className="text-center">
                    <button 
                      onClick={() => setSelectedOrderId(txn.order_id)}
                      className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-primary text-accent hover:scale-110 transition-transform shadow-sm"
                    >
                      <Eye size={14} />
                    </button>
                  </div>
