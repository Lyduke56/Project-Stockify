...
export default function TransactionsTable({ onLoadComplete, colors }: TransactionsTableProps) {
    ...
    <div className="w-full flex flex-col font-['Inter'] px-6 space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard 
          ...
          className="w-full"
          iconClassName="w-13 h-13 text-primary"
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="w-full flex justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setSortAsc(!sortAsc)}
              className="flex items-center gap-2 border-[1.5px] border-primary text-primary px-4 py-2.5 rounded-full text-[13px] font-bold hover:bg-primary/5 transition-colors">
              <ArrowUpDown size={14} /> {sortAsc ? "Oldest First" : "Newest First"}
            </button>
            <button onClick={handleRefresh} disabled={refreshing}
              className="p-2.5 rounded-full border-[1.5px] border-primary text-primary hover:bg-primary/10 transition-all disabled:opacity-50">
        ...
        <div className="w-full bg-transparent rounded-[10px] border border-primary/30 flex flex-col overflow-hidden shadow-sm">
          ...
          <div className="w-full grid grid-cols-[1fr_1fr_2fr_1.5fr_1.5fr_1fr_1fr] bg-primary px-4 py-3 rounded-t-[8px]">
            {COLUMNS.map((col) => (
              ...
              {displayedTxns.map((txn, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`w-full grid grid-cols-[1fr_1fr_2fr_1.5fr_1.5fr_1fr_1fr] px-4 py-[14px] items-center hover:bg-primary/5 transition-colors ${!isLast ? "border-b border-primary/10" : ""}`}
                  >
                    ...
                    <div className="flex justify-center">
                      <span className="text-[11px] font-bold bg-primary/10 text-primary px-2.5 py-1 rounded-full whitespace-nowrap">
                        {txn.payment_method === "QR Code" ? "QR Code" : txn.payment_method === "Cash-on-Delivery" ? "Cash on Delivery" : txn.payment_method}
                      </span>
                    </div>
                    
                    <div className="text-center text-primary text-[13px] font-black">
                    ...
                    <div className="flex justify-center">
                      <button 
                        ...
                        className="px-4 py-1.5 rounded-full bg-accent text-primary text-[11px] font-black hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm"
                      >
            ...
            <div className="flex items-center gap-3">
              ...
              {hasMore && (
                <button 
                  ...
                  className="px-6 py-2.5 rounded-full bg-primary text-sidebar-text font-bold text-[13px] hover:opacity-90 shadow-sm transition-all flex items-center gap-2"
                >
