      ...
      <div ref={tableRef} className="w-full bg-[#FFFCEB] rounded-[10px] border border-[#385E31] flex flex-col overflow-visible shadow-sm">
        ...
        {loading ? (
          ...
          displayed.map((row, idx) => {
            ...
            const defaultSize = row.sizes?.find((s) => s.is_default);
            const displayPrice = defaultSize ? Number(defaultSize.price) : Number(row.price);
            ...
            return (
              <div key={row.product_id} className={`w-full flex px-2 py-[10px] items-center ${!isLast ? "border-b border-[#385E31]/20" : ""}`}>
                ...
                <div className={`flex items-center ${COLUMNS[1].className}`}>
                  {row.image_url ? (
                    <img src={row.image_url} alt={row.name} className="w-8 h-8 object-cover rounded border border-[#385E31]" />
                  ) : (
                    <div className="w-8 h-8 border border-[#385E31] rounded bg-white flex items-center justify-center"><span className="text-[#385E31]/50 text-[9px] font-bold">IMG</span></div>
                  )}
                </div>
                {/* SKU */}
                <div className={`flex text-[#3A6131] text-[12px] font-bold font-mono items-center ${COLUMNS[2].className}`}>{row.sku}</div>
                {/* Category */}
