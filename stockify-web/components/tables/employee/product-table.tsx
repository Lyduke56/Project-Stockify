...
import ProductModal from "@/components/modals/employee/ingredients-modals/product-modal";
...
export default function ProductsTable({ tenantId }: ProductsTableProps) {
  ...
  const handleAdd = async (
    input: Parameters<typeof addProduct>[1],
    recipe: RecipeInput[],
    imageFile: File | null
  ) => {
    // 1. Insert product (no image_url yet)
    const product = await addProduct(tenantId, { ...input, image_url: null }, recipe);

    // 2. Upload image if provided, then update the URL
    if (imageFile) {
      const url = await uploadProductImage(tenantId, product.product_id, imageFile);
      await updateProduct(product.product_id, tenantId, { image_url: url }, recipe);
    }
  ...
  const handleEdit = async (
    ...
    let image_url = input.image_url; // keep existing if no new file

    if (imageFile) {
      image_url = await uploadProductImage(tenantId, editTarget.product_id, imageFile);
    }

    await updateProduct(editTarget.product_id, tenantId, { ...input, image_url }, recipe);
    setEditTarget(null);
    loadProducts();
  };
          ...
          displayed.map((row, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className={`flex items-center ${COLUMNS[1].className}`}>
                  {row.image_url ? (
                    <img
                      src={row.image_url}
                      alt={row.name}
                      className="w-8 h-8 object-cover rounded border border-[#385E31]"
                    />
                  ) : (
                ...
                {/* SKU */}
                <div className={`flex text-[#3A6131] text-[12px] font-bold font-mono items-center ${COLUMNS[2].className}`}>
                  {row.sku}
                </div>
                ...
                {/* Max Yield */}
                <div className={`flex text-[13px] font-bold items-center ${COLUMNS[6].className} ${row.max_yield <= 10 ? "text-[#E91F22]" : "text-[#3A6131]"}`}>
                  {row.max_yield}
                </div>
