    ...
    <div className="w-full flex flex-col font-['Inter']">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex-1 h-full z-20 text-center font-bold text-[16px] transition-colors duration-300 cursor-pointer ${isActive ? TAB_META[tab].text : "text-primary"
                  }`}
              >
      ...
      <div className="w-full bg-background rounded-[10px] border border-primary flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="w-full flex bg-primary px-4 py-3 rounded-t-[8px]">
          {COLUMNS.map((col) => (
            <div key={col} className="flex-1 text-center text-[#FFFCEB] text-[12px] font-bold">
              {col}
            </div>
          ))}
        </div>
